...
# -----------------------------------------------------------------------------
# Patched: Reverted to a single-file structure for simplicity. Includes all recent fixes and UI improvements.
# Timestamp: 2025-09-16
# Filename: app_single_file_v7.1.py
# -----------------------------------------------------------------------------
# Version: 7.1
# -----------------------------------------------------------------------------
                ...
                if st.form_submit_button("Add Department"):
                    if new_dept_name:
                        try:
                            ...
                            st.toast(f"Added '{new_dept_name.strip()}'.", icon="✅"); get_departments.clear();
                        except Exception as e:
                        ...
                        with st.form("enrollment_form"):
                            ...
                            if st.form_submit_button("Update Enrollment"):
                                try:
                                    ...
                                    st.toast("Enrollment updated.", icon="✅")
                        ...
                        if uploaded_file:
                            try:
                                ...
                                with st.spinner(f"Uploading {len(df)} records to '{table}'..."):
                                    ...
                                    st.toast(f"Uploaded {len(df)} records.", icon="🎉")
                    ...
                    if not is_danger_unlocked():
                        ...
                        if st.button("Permanently Delete All Students", type="primary", disabled=not is_danger_unlocked()):
                            try:
                                ...
                                st.toast("All student records deleted.", icon="🚨"); load_roster.clear();
                            except Exception as e: st.error(f"Error: {e}")
                        if st.button("Permanently Delete All Faculty", type="primary", disabled=not is_danger_unlocked()):
                            try:
                                ...
                                st.toast("All faculty records deleted.", icon="🚨")
                            except Exception as e: st.error(f"Error: {e}")
                        if st.button("Permanently Delete All Courses", type="primary", disabled=not is_danger_unlocked()):
                            try:
                                ...
                                st.toast("All course records deleted.", icon="🚨"); get_courses.clear();
                            except Exception as e: st.error(f"Error: {e}")
