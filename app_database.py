...
# -----------------------------------------------------------------------------
# Patched: Updated bulk upload UI to use vertical radio buttons for clarity.
# Timestamp: 2025-09-16
# Filename: app_final_ui_2025-09-16_v6.9.py
# -----------------------------------------------------------------------------
# Version: 6.9
# -----------------------------------------------------------------------------
...
def update_course_enrollment(course_details: dict, student_ids: list):
    """Deletes and inserts enrollment records for a given course."""
    match_criteria = {
        'course_code': course_details['course_code'],
        'department_id': course_details['department_id'],
        'class_name': course_details['class_name'],
        'section': course_details['section']
    }
    supabase.table('student_course_enrollment').delete().match(match_criteria).execute()
    if student_ids:
        records_to_insert = [{**match_criteria, 'student_id': sid} for sid in student_ids]
        supabase.table('student_course_enrollment').insert(records_to_insert).execute()
        ...
        if st.session_state.faculty_course_selection is None:
            ...
            if courses_df.empty:
                ...
                for i in range(3):
                    ...
                    recent_months.append({"display_name": f"{month_name} {year}", "month_yyyy_mm": month_key(month_name, year)})
                ...
                for index, course_row in courses_df.iterrows():
                    ...
                    with st.container(border=True):
                        ...
                        st.caption(f"{dept_name} — {course_row['class_name']} / {course_row['section']}")
                        stat_cols = st.columns(len(recent_months))
                        
                        for i, month_info in enumerate(recent_months):
                            with stat_cols[i]:
                                status_entry = merged_df[(merged_df['course_code'] == course_row['course_code']) & (merged_df['section'] == course_row['section']) & (merged_df['month_yyyy_mm'] == month_info['month_yyyy_mm'])]
                                status = status_entry['status'].iloc[0] if not status_entry.empty and not pd.isna(status_entry['status'].iloc[0]) else "Not Started"
                ...
                with st.expander("🪢 Manage Enrollment for this Course"):
                    ...
                    if st.button("Update Enrollment", key=f"update_enroll_{course['course_code']}"):
                        try:
                            ...
                            st.success("Enrollment updated successfully!"); st.rerun()
                        ...
                        with st.form("enrollment_form"):
                            ...
                            if st.form_submit_button("Update Enrollment"):
                                try:
                                    ...
                                    st.success("Enrollment updated."); st.rerun()
                        ...
                        if uploaded_file:
                            try:
                                ...
                                if id_col and id_col in df.columns and df[id_col].duplicated().any():
                                    ...
                                    st.error(f"Upload failed. Duplicate IDs found in '{id_col}' column: {', '.join(duplicates)}")
                                    st.stop()
                                df.rename(columns=str.lower, inplace=True)
                                if 'prn' in df.columns: df.rename(columns={'prn': 'PRN'}, inplace=True)
